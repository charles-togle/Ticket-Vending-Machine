...
export default function Home(): React.ReactNode {
  ...
  const [isChooseStation, setIsChooseStation] = useState(false);
  const [station, setStation] = useState("Recto");
  ...
  const handleConfirmPromptSingle = useCallback(() => {
    setIsChooseStation(false);
  ...
  }, [setSingleJourneyTicket, station, navigate]);
  ...
  return (
    <section className="w-screen h-screen relative">
      ...
      {isChooseStation && (
        <Prompt
          promptText="Choose current Station"
          setInputValue={setStation}
          cancelAction={() => setIsChooseStation(false)}
          confirmAction={handleConfirmPromptSingle}
          isDropdown={true}
          dropdownContent={stationList}
        ></Prompt>
      )}
      ...
      <main className="flex justify-center items-center h-full space-x-10 pt-30">
        <div className="h-[70vh] w-[40vw] border-10 border-blue-900 text-center p-10 flex flex-col items-center font-semibold">
          ...
          <button
            ...
            onClick={()=>setIsChooseStation(true)}
          >
